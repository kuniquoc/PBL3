    ...
    */
    ...
    public ActionResult GetAccountByCookie()
    {
      try
      {
        ...
        // Get account by id
        var account = _accountDAO.GetAccountById(int.Parse(claims["id"]));
        if (account == null)
        {
    ...
    */
    ...
    public ActionResult UpdateAccount([FromBody] AccountUpdateModel account)
    {
      try
      {
        var claims = _accountBLL.GetClaimsByCookie(HttpContext);
        // Get account by id
        var accountDB = _accountDAO.GetAccountById(int.Parse(claims["id"]));
        if (accountDB == null)
        {
          return NotFound(new { message = "User not found" });
        }
    ...
    */
    ...
    public ActionResult UserList(string? search, int page = 1, int limit = 15, string role = "user", string sortBy = "created_at", string sortType = "desc")
    {
      ...
      try
      {
        ...
        // Search for accounts
        var result = _accountDAO.SearchAccount(search, page, limit, role, sortBy, sortType);
