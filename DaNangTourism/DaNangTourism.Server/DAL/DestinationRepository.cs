...
namespace DaNangTourism.Server.DAL
{
  ...
  public class DestinationRepository : IDestinationRepository
  {
    ...
    public DestinationDetail? GetDestinationById(int id, int userId = 0)
    {
      ...
      if (userId > 0)
      {
        sql.Append(", IF(FavoriteDestinations.UserId IS NOT NULL, TRUE, FALSE) as Favorite FROM Destinations LEFT JOIN (SELECT * FROM FavoriteDestinations WHERE UserId = @userId) AS FavoriteDestinations ON FavoriteDestinations.DestinationId = Destinations.DestinationId ");
